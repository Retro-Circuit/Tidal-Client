...
public final class KeystrokesHud {
    ...
    private static void drawKey(GuiGraphics graphics, Font font, int x, int y, int w, int h, String label, float press) {
        int color = press > 0.5f ? 0xFF3D6FFF : Glass.ACCENT;
        Glass.roundedFill(graphics, x, y, x + w, y + h, 5, color);
        graphics.drawCenteredString(font, label, x + w / 2, y + (h - 8) / 2, Glass.TEXT);
    }
}
