...
public class PerformanceMenuScreen extends TidalPanelScreen {
    ...
    protected void drawBody(GuiGraphics graphics, int mouseX, int mouseY, float delta) {
        ...
        graphics.drawString(this.font, "Master", this.bodyX + 12, y + 8, Glass.TEXT, false);
        graphics.drawString(this.font, "Auto-tunes around Sodium, Iris, and friends", this.bodyX + 12, y + 20, Glass.MUTE, false);
        ...
        for (TidalBoost.Pass pass : TidalBoost.passes()) {
            ...
            graphics.drawString(this.font, this.fit(pass.title(), this.bodyW - 24), this.bodyX + 12, y + 6, Glass.TEXT, false);
            graphics.drawString(this.font, this.fit(pass.hint(), this.bodyW - 24), this.bodyX + 12, y + 17, Glass.MUTE, false);
            y += 38;
        }
        y = this.section(graphics, y, "Detected");
        List<TidalBoost.Companion> companions = TidalBoost.companions();
        if (companions.isEmpty()) {
            graphics.drawString(this.font, "Vanilla renderer — Tidal covers particles, clouds, weather, entities, FPS.", this.bodyX + 4, y + 6, Glass.MUTE, false);
            return;
        }
        for (TidalBoost.Companion companion : companions) {
            graphics.drawString(this.font, companion.name() + "  ·  Tidal yields overlapping work", this.bodyX + 4, y + 2, Glass.TEXT, false);
            y += 16;
        }
    }
