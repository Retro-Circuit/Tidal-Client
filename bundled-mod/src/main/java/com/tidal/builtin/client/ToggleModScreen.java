...
public class ToggleModScreen extends TidalPanelScreen {
    ...
    protected void drawBody(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        ...
        graphics.text(this.font, this.fit(this.label, this.bodyW - 58), this.bodyX + 12, this.bodyY + 10, Glass.TEXT, false);
        Glass.toggle(graphics, this.bodyX + this.bodyW - 42, this.bodyY + 6, on);
        graphics.text(this.font, "HUD modules can be moved and scaled in Layout.", this.bodyX + 4, this.bodyY + 40, Glass.MUTE, false);
    }
