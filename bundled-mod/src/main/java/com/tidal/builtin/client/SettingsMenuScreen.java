...
public class SettingsMenuScreen extends TidalPanelScreen {
    ...
    protected void drawBody(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        ...
        for (int i = 0; i < SIDE.length; i++) {
            ...
            graphics.text(this.font, SIDE[i], this.bodyX + 16, y + 7, on ? Glass.TEXT : Glass.MUTE, false);
        }
        ...
        if (this.side == 3) {
            ...
            graphics.text(this.font, this.fit("Tidal menu", labelW), contentX + 12, rowY + 10, Glass.TEXT, false);
            String bind = this.listening ? "Press a key..." : TidalMods.menuKeyName();
            graphics.text(this.font, this.fit(bind, 80), contentX + contentW - Math.min(80, this.font.width(bind)) - 12, rowY + 10, this.listening ? Glass.ACCENT : Glass.MUTE, false);
            return;
        }
        ...
        for (int i = 0; i < labels.length; i++) {
            ...
            graphics.text(this.font, this.fit(labels[i], labelW), contentX + 12, rowY + 10, Glass.TEXT, false);
    ...
    private void drawPerformance(GuiGraphicsExtractor graphics, int x, int w, int y) {
        Glass.row(graphics, x, y, w, 32, false);
        graphics.text(this.font, "Tidal Boost", x + 12, y + 6, Glass.TEXT, false);
        graphics.text(this.font, "Auto-tunes around detected performance mods", x + 12, y + 17, Glass.MUTE, false);
        Glass.toggle(graphics, x + w - 42, y + 8, TidalMods.boost);
        y += 40;
        for (TidalBoost.Pass pass : TidalBoost.passes()) {
            graphics.text(this.font, this.fit((pass.active() ? "● " : "○ ") + pass.title(), w - 8), x + 4, y, Glass.TEXT, false);
            y += 12;
            graphics.text(this.font, this.fit(pass.hint(), w - 8), x + 4, y, Glass.MUTE, false);
            y += 16;
        }
        graphics.text(this.font, "DETECTED", x + 4, y + 4, Glass.MUTE, false);
        y += 18;
        var companions = TidalBoost.companions();
        if (companions.isEmpty()) {
            graphics.text(this.font, "None — Tidal covers the gaps.", x + 4, y + 6, Glass.MUTE, false);
            return;
        }
        for (TidalBoost.Companion companion : companions) {
            graphics.text(this.font, companion.name() + "  ·  active", x + 4, y + 2, Glass.TEXT, false);
            y += 16;
        }
    }
