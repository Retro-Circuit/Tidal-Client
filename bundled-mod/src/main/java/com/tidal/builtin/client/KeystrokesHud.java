...
public final class KeystrokesHud {
    private static final Identifier DEFAULT_IDLE = Identifier.fromNamespaceAndPath("tidal-builtin", "dynamic/keystrokes_default");
    private static final Identifier DEFAULT_DOWN = Identifier.fromNamespaceAndPath("tidal-builtin", "dynamic/keystrokes_default_down");
    private static final Identifier INFO_IDLE = Identifier.fromNamespaceAndPath("tidal-builtin", "dynamic/keystrokes_info");
    private static final Identifier INFO_DOWN = Identifier.fromNamespaceAndPath("tidal-builtin", "dynamic/keystrokes_info_down");
    ...
    static void draw(GuiGraphicsExtractor graphics, Minecraft minecraft, int originX, int originY, boolean editing) {
        ...
        Identifier idle = TidalMods.keystrokesInfo ? INFO_IDLE : DEFAULT_IDLE;
        Identifier pressedTex = TidalMods.keystrokesInfo ? INFO_DOWN : DEFAULT_DOWN;
        for (Key key : keys) {
            ...
            if (pressed) {
                graphics.blit(pressedTex, x, y, x + w, y + h, key.pu0, key.pu1, key.pv0, key.pv1);
            } else {
                graphics.blit(idle, x, y, x + w, y + h, key.u0, key.u1, key.v0, key.v1);
            }
        }
    }
    ...
    private static void register(Minecraft minecraft, Identifier id, String classpath) {
        try (InputStream stream = KeystrokesHud.class.getResourceAsStream(classpath)) {
            ...
            try {
                ...
                java.awt.image.BufferedImage buffered = javax.imageio.ImageIO.read(KeystrokesHud.class.getResourceAsStream(classpath));
                if (buffered == null) {
                    return;
                }
                java.io.ByteArrayOutputStream out = new java.io.ByteArrayOutputStream();
                javax.imageio.ImageIO.write(buffered, "png", out);
                image = NativeImage.read(new java.io.ByteArrayInputStream(out.toByteArray()));
            }
