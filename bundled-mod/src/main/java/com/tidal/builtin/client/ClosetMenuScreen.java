...
public class ClosetMenuScreen extends TidalPanelScreen {
    ...
    protected void drawBody(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        ...
        if (!this.capesTab) {
            ...
            graphics.text(this.font, this.fit(AccountCapes.status(), listW - 8), this.bodyX + 124, this.scrolledY() + 5, Glass.MUTE, false);
        }
        ...
        if (this.capesTab) {
            ...
            for (int i = 0; i < items.size(); i++) {
                ...
                graphics.text(this.font, this.fit(item.name, listW - 80), this.bodyX + 34, y + 8, Glass.TEXT, false);
                graphics.text(this.font, on ? "Equipped" : item.hint, this.bodyX + 34, y + 20, Glass.MUTE, false);
                y += ROW;
            }
            if (hover >= 0) {
                this.selected = hover;
            ...
            graphics.text(this.font, "Minecraft account", this.bodyX + 12, y + 8, Glass.TEXT, false);
            graphics.text(this.font, defOn ? "Equipped" : "Default skin", this.bodyX + 12, y + 20, Glass.MUTE, false);
            if (defHot) {
                hover = 0;
            }
            y += ROW;
            Glass.row(graphics, this.bodyX, y, listW, ROW - 6, false);
            graphics.text(this.font, LocalSkins.status(), this.bodyX + 12, y + 12, Glass.MUTE, false);
            y += ROW;
            ...
            for (int i = 0; i < skins.size(); i++) {
                ...
                graphics.text(this.font, this.fit(entry.name(), listW - 80), this.bodyX + 36, y + 8, Glass.TEXT, false);
                graphics.text(this.font, on ? "Equipped" : entry.slim() ? "Alex" : "Steve", this.bodyX + 36, y + 20, Glass.MUTE, false);
                y += ROW;
            }
            if (hover >= 0) {
                this.selected = hover;
    ...
    private void tab(GuiGraphicsExtractor graphics, int x, int y, int w, boolean on, String label) {
        Glass.pill(graphics, x, y, w, 18, on ? 0x660349FC : 0x33000000);
        graphics.centeredText(this.font, label, x + w / 2, y + 5, on ? Glass.TEXT : Glass.MUTE);
    }

    private void drawPreview(GuiGraphicsExtractor graphics, int mouseX, int mouseY, int previewX) {
        int x0 = previewX + 4;
        int y0 = this.bodyY + 6;
