...
public final class ArmorHud {
    ...
    static void draw(GuiGraphicsExtractor graphics, Minecraft minecraft, int x, int y, boolean editing) {
        ...
        for (int i = 0; i < SLOTS.length; i++) {
            ...
            if (!empty && TidalMods.armorPercent) {
                ...
                graphics.text(minecraft.font, pct + "%", cx + (TidalMods.armorIcons ? 18 : 0), cy + 4, color, false);
            }
        ...
        if (TidalMods.armorPoints) {
            graphics.text(minecraft.font, String.valueOf(player.getArmorValue()), x, TidalMods.armorVertical ? cy : y + 16, Glass.MUTE, false);
        }
    }
}
