...
public final class CapeShare {
    ...
    private static final AtomicInteger HTTP_INFLIGHT = new AtomicInteger();
    ...
    public static ClientAsset.Texture asset(UUID uuid) {
        String id = REMOTE.get(uuid);
        if (id == null || id.isBlank() || id.startsWith("ms:")) {
            return null;
        }
        return Capes.assetById(id);
    }

    public static void receive(Identifier identifier) {
        if (identifier == null || !"tidal-builtin".equals(identifier.getNamespace())) {
            return;
        }
    ...
    private static void sendPayload(Minecraft minecraft, UUID uuid, String capeId) {
        ...
        Identifier identifier = Identifier.fromNamespaceAndPath("tidal-builtin", "cape/" + id + "/" + uuid.toString().replace("-", ""));
        try {
            connection.send(new ServerboundCustomPayloadPacket(new DiscardedPayload(identifier)));
        } catch (Exception ignored) {
        }
    }
